...
namespace engine
{
  ...
  struct ENGINE_API fgraphics_command_list
  {
    ...
    void set_render_targets(const frtv_resource& rtv, const fdsv_resource& dsv) const;
    void set_viewport(uint32_t width, uint32_t height) const;
    void set_scissor(uint32_t width, uint32_t height) const;

    void clear_render_target(const frtv_resource& rtv) const;
    void clear_depth_stencil(const fdsv_resource& dsv) const;

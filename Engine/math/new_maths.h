...
namespace engine
{
  ...
  {
    return degs * (static_cast<float>(math::pi) / 180.0f);
  }
  ...
  inline float4x4 makePerspectiveMat(float aspectRatio, float fovYRadians, float zNear, float zFar)
  {
    ...
    const float yScale = tanf(0.5f * (static_cast<float>(math::pi) - fovYRadians));
    const float xScale = yScale / aspectRatio;
    const float zRangeInverse = 1.f / (zNear - zFar);
    const float zScale = zFar * zRangeInverse;
    const float zTranslation = zFar * zNear * zRangeInverse;

...
#define OBJECT_REGISTRY_EXPLICIT_INSTANTIATE(CLASS_NAME) \
  ...
  template ENGINE_API const CLASS_NAME* object_registry::find_const<CLASS_NAME>(const std::string& name); \
...
#define CLASS_OBJECT_EXPLICIT_INSTANTIATE(CLASS_NAME) \
  template ENGINE_API CLASS_NAME* class_object::spawn_instance(const std::string& name) const;
...
#define CLASS_OBJECT_REGISTER(CLASS_NAME, PARENT_CLASS_NAME) \
  { \
    ...
    obj->spawn_instance_func = [](const std::string& name) -> object* { return CLASS_NAME::spawn(name); }; \
    register_class(obj); \
  }

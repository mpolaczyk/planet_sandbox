...
namespace engine
{
  ...
  void rcpu_reference::worker_job_update()
  {
    ...
    fchunk_generator::generate_chunks(chunk_strategy_type::rectangles, std::thread::hardware_concurrency() * chunks_per_thread, job_state.image_width, job_state.image_height, chunks);

    concurrency::parallel_for_each(begin(chunks), end(chunks), [&](const fchunk& ch) { render_chunk(ch); });
  }


  void rcpu_reference::render_chunk(const fchunk& in_chunk)
  {
    ...
    fvec3 resolution((float)job_state.image_width, (float)job_state.image_height, 0.0f);
    ...
    for (int y = in_chunk.y; y < in_chunk.y + in_chunk.size_y; ++y)
    {
      for (int x = in_chunk.x; x < in_chunk.x + in_chunk.size_x; ++x)
      {
        ...
        fvec3 hdr_color = fragment((float)x, (float)y, resolution);

        assert(isfinite(hdr_color.x));
        assert(isfinite(hdr_color.y));
        assert(isfinite(hdr_color.z));

        fvec3 ldr_color = tone_mapping::reinhard_extended(hdr_color, job_state.renderer_conf.white_point);

        fbmp_pixel p(ldr_color);
  ...
  fvec3 rcpu_reference::fragment(float x, float y, const fvec3& resolution)
  {
    ...
    fvec3 hdr_color = sum_colors / (float)rays_per_pixel;

    return hdr_color;
  }

  fvec3 rcpu_reference::enviroment_light(const fray& in_ray)
  {
    static const fvec3 sky_color_zenith = fcolors::white_blue;
    static const fvec3 sky_color_horizon = fcolors::white;
    static const float sky_brightness = 0.4f;

    float t = fmath::smoothstep(-0.6f, 0.2f, in_ray.direction.y);
    fvec3 light = fmath::lerp_vec3(sky_color_horizon, sky_color_zenith, t);
    return fmath::clamp_vec3(0.0f, 1.0f, light) * sky_brightness;
  }

  fvec3 rcpu_reference::trace_ray(fray in_ray, uint32_t seed)
  {
    ...
    fvec3 incoming_light = fvec3(0.0f);
    ...
    for (int i = 0; i < bounces; ++i)
    {
      fstats::inc_ray();
      fhit_record hit;
      if (job_state.scene_root->hit(in_ray, fmath::infinity, hit))
      {
        // Don't bounce if ray has no color
        if (fmath::length_squared(ray_color) < 0.1f)
        {
          break;
        }

        // Read material
        amaterial mat = *hit.material_ptr;
        ...
        bool can_gloss_bounce = mat.gloss_probability >= frandom_seed::rand_pcg(seed);
        bool can_refract = mat.refraction_probability >= frandom_seed::rand_pcg(seed);

        fvec3 diffuse_dir = fmath::normalize(hit.normal + frandom_seed::direction(seed));

        if (!can_gloss_bounce && can_refract)
        {
          float refraction_ratio = hit.front_face ? (1.0f / mat.refraction_index) : mat.refraction_index;
          fvec3 refraction_dir = fmath::refract(in_ray.direction, hit.normal, refraction_ratio);
          ...
          assert(fcolors::is_valid(ray_color));
          continue;
        }

        // New directions

        fvec3 specular_dir = fmath::reflect(in_ray.direction, hit.normal);
        ...
        in_ray.direction = fmath::lerp_vec3(diffuse_dir, specular_dir, blend);

        // Calculate color for this hit
        incoming_light += mat.emitted_color * ray_color;
        ray_color *= fmath::lerp_vec3(mat.color, mat.gloss_color, can_gloss_bounce);
        assert(fcolors::is_valid(ray_color));
      }
      else
      {
        fvec3 env_light = enviroment_light(in_ray);  // environment is emissive

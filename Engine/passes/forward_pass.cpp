...
namespace engine
{
  ...
  void fforward_pass::init()
  {
    ...
    srv_lights_heap_index = cbv_frame_data_heap_index + context->window->back_buffer_count;
    srv_materials_heap_index = srv_lights_heap_index + context->window->back_buffer_count;
    srv_textures_heap_index = srv_materials_heap_index + context->window->back_buffer_count;
    ...
    // Create frame data CBV
    {
      ...
      for(uint32_t i = 0; i < context->window->back_buffer_count; i++)
      {
        CD3DX12_CPU_DESCRIPTOR_HANDLE handle(context->window->main_descriptor_heap->GetCPUDescriptorHandleForHeapStart(), cbv_frame_data_heap_index + i, main_descriptor_size);
    ...
    // Create light and material data SRV
    {
      ...
      {
        {
          CD3DX12_CPU_DESCRIPTOR_HANDLE handle(context->window->main_descriptor_heap->GetCPUDescriptorHandleForHeapStart(), srv_lights_heap_index + i, main_descriptor_size);
          ComPtr<ID3D12Resource> resource;
        ...
        {
          CD3DX12_CPU_DESCRIPTOR_HANDLE handle(context->window->main_descriptor_heap->GetCPUDescriptorHandleForHeapStart(), srv_materials_heap_index + i, main_descriptor_size);
          ComPtr<ID3D12Resource> resource;
  ...
  void fforward_pass::draw(ComPtr<ID3D12GraphicsCommandList> command_list)
  {
    ...
    // Process texture SRVs
    {
      ...
      CD3DX12_CPU_DESCRIPTOR_HANDLE handle(context->window->main_descriptor_heap->GetCPUDescriptorHandleForHeapStart(), srv_textures_heap_index, descriptor_size);
      ...
      {
        fdx12::upload_texture_buffer(device, command_list, context->window->main_descriptor_heap, handle, default_texture);
      ...
      for(uint32_t i = 0; i < MAX_TEXTURES-1; i++)
      {
        if(i < num_textures_in_scene && context->scene_acceleration->a_textures[i] != default_texture)
        {
          ...
          {
            fdx12::upload_texture_buffer(device, command_list, context->window->main_descriptor_heap, handle, texture);
            handle.Offset(descriptor_size);
    ...
    for(uint32_t i = 0; i < N; i++)
    {
      ...
      command_list->SetGraphicsRootDescriptorTable(root_parameter_type::textures, context->window->main_descriptor_heap->GetGPUDescriptorHandleForHeapStart());

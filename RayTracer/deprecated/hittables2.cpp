...
void scene::query_lights()
{
  ...
  for (hittable* object : objects)
  {
    const engine::material_asset* mat = object->material_asset.get();
...
void static_mesh::pre_render()
{
  ...
  runtime_asset = engine::get_object_registry()->find<engine::static_mesh_asset>(oss.str());
  if (runtime_asset == nullptr)
  {
    if (engine::get_object_registry()->find<engine::static_mesh_asset>(mesh_asset.get_name()) != nullptr)
    {
      runtime_asset = engine::get_object_registry()->clone<engine::static_mesh_asset>(mesh_asset.get()->get_runtime_id(), oss.str());
    }
    else
    {
      return;
